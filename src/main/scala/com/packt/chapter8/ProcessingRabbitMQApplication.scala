...
object ProcessingRabbitMQApplication extends App {
  ...
  val amqpSinkConfig = AmqpSinkSettings(connectionSetting,
    Some(exchangeName), None, sinkDeclarations)

  val rabbitSinkConfig = AmqpSinkSettings(connectionSetting,Some(exchangeName),None,sinkDeclarations)
  ...
  val stream = rabbitMQSource
    .map(incomingMessage => {
      val upperCased =
        incomingMessage.bytes.utf8String.toUpperCase
