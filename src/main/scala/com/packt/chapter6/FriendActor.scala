...
object FriendActor {
  def props(friendId: String, recovery: Recovery) = Props(new
      FriendActor(friendId, recovery))
}

class FriendActor(friendId: String, r: Recovery) extends
  PersistentActor with ActorLogging {
  override val persistenceId = friendId
  override val recovery = r
  var state = FriendState()

  def updateState(event: FriendEvent) = state =
    state.update(event)
